...
for (i in 2015:2019)
{
  ...
  # health districts
  con <- get_db_conn(db_pass = Sys.getenv("db_pwd"))
...
```{r}
con <- get_db_conn(db_pass = Sys.getenv("db_pwd"))
A <- st_read(con, query = "SELECT * FROM dc_education_training.va_tr_acs_2015_2019_post_hs_education")
B <- st_read(con, query = "SELECT * FROM dc_education_training.va_tr_sdad_2021_post_hs_education")
dbDisconnect(con)
